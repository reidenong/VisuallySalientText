...
def load_list(dataset_name, data_root):
    ...
    img_root = data_root + dataset_name + '/OCSD-TR-Image/'
    img_files = os.listdir(img_root)

    for img in img_files:

        images.append(img_root + img[:-4]+'.jpg')
        labels.append(img_root.replace('/OCSD-TR-Image/', '/OCSD-TR-Mask/') + img[:-4]+'.png')
        contours.append(img_root.replace('/OCSD-TR-Image/', '/OCSD-TR-Contour/') + img[:-4] + '.png')
...
def load_test_list(test_path, data_root):
    ...
    if 'OCSD' in test_path:
        img_root = data_root + test_path + '/OCSD-TE-Image/'
    else:
